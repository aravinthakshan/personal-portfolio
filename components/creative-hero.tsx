  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      constructor(x: number, y: number) {
        ...
        // Create a gradient from sage to beige
        const hue = Math.random() * 30 + 80 // 80-110 range for greens and beiges
        this.color = `hsl(${hue}, 40%, 60%)`
      }
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particlesArray.length; i++) {
        ...
        for (let j = i; j < particlesArray.length; j++) {
          ...
          if (distance < 30) {
            ctx.beginPath()
            ctx.strokeStyle = `rgba(163, 182, 138, ${0.3 - distance / 150})` // sage color
            ctx.lineWidth = 0.5
